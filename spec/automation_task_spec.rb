...
=end
  ...
  before :all do
    ...
    @driver = Watir::Browser.new :chrome, listener: Listener.new
  end
  ...
  it 'Check that the status can be changed of already created Leads too' do
    ...
    ##@leads_page.select_created_lead("4zq8nsew19")

    # puts 'TEST open created Lead'
    # @leads_page.select_created_lead('echn8xp5y7')
    # #@leads_page.some
    # expect(@leads_page.lead_status).to eql('echn8xp5y7')
    ...
    #@leads_page = @settings_page.top_bar.select_leads_tab #is it correct?
    @leads_page = @leads_page.top_bar.select_leads_tab
